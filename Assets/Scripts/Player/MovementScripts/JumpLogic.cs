...
public class JumpLogic : MonoBehaviour
{
    ...
    {
        _isOnGround = Physics2D.OverlapBox(transform.position, _overlapBoxSize, groundMask);
        
        var targetMaterial = _isOnGround ? groundFriction : airFriction;
        ...
        if (_shouldJump && _isOnGround && !_crouchLogic.isCrouching)
        {
